...
2. **터미널에서 백엔드 프로젝트 경로를 지정하세요**
    ```
    cd './backend 로 경로 지정(다운받은 backend폴더를 열고 경로 복사)'
    ```
...
1. **계산에 시간이 많이 소요될 수 있습니다.**
   - 초기 계산은 약 10분가량 소요(코어6개 기준), 반복 계산 시 시간이 크게 단축됨 (캐시사용)
   - 보유한 GPU 성능에 따라 계산 시간 차이 존재할 수 있음

2. **정확도 수정 시 계산량 증가로 인하여 결과가 제대로 출력되지 않을 수 있습니다.**
   - 정확도(시뮬레이션 횟수)는 100회에서 수정하지 않는 것을 권장합니다.
